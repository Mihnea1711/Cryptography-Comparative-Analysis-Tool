...
from business.algorithm.openssl.aes import AESCipher_OpenSSL
from business.algorithm.openssl.rsa import RSACipher_OpenSSL
...
class OpenSSLService(ICryptoFramework):
    ...
    def set_algorithm(self, algorithm: Algorithm):
        if algorithm.algorithm_name == AES_ALGORITHM:
            self.algorithm = AESCipher_OpenSSL(db_agent=self.get_db_agent(), algorithm_name=algorithm.algorithm_name)
        elif algorithm.algorithm_name == RSA_ALGORITHM:
            self.algorithm = RSACipher_OpenSSL(db_agent=self.get_db_agent(), algorithm_name=algorithm.algorithm_name)
        else:
